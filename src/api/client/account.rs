...
pub(crate) async fn get_register_available_route(
	SecureClientIp(ip): SecureClientIp, body: Ruma<get_username_availability::v3::Request>,
...
pub(crate) async fn register_route(
	SecureClientIp(ip): SecureClientIp, body: Ruma<register::v3::Request>,
	...
	if body.appservice_info.is_none() && !is_guest {
		...
		services()
			...
			.send_message(RoomMessageEventContent::notice_plain(format!(
				"New user \"{user_id}\" registered on this server from IP {ip}."
			)))
			.await;
	}
	...
	if body.appservice_info.is_none() && is_guest && services().globals.log_guest_registrations() {
		info!("New guest user \"{user_id}\" registered on this server from IP {ip}.");
		...
		if let Some(device_display_name) = &body.initial_device_display_name {
				...
				.is_some_and(|device_display_name| !device_display_name.is_empty())
			{
				...
				services()
					.admin
					.send_message(RoomMessageEventContent::notice_plain(format!(
						"Guest user \"{user_id}\" with no device display name registered on this server from IP {ip}.",
					)))
					.await;
			}
		} else {
...
pub(crate) async fn change_password_route(
	SecureClientIp(ip): SecureClientIp, body: Ruma<change_password::v3::Request>,
...
pub(crate) async fn deactivate_route(
	SecureClientIp(ip): SecureClientIp, body: Ruma<deactivate::v3::Request>,
